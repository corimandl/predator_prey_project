...
- updated the order for computing the angular velocity
"""
import os
from structs import *
...
class Sheep(Agent):
    ...
    def step_agent(agent, input, step_params):
        ...
        def step_active_agent():
            ...
            # calculate linear velocities based on NEW angle
            ang_dot_new = ang_speed - dt * ang_dot * damping
            ang_new = jnp.mod(ang + dt*ang_dot_new + jnp.pi, 2*jnp.pi) - jnp.pi

            x_dot_new = speed * jnp.cos(ang_new) - dt * x_dot * damping
            y_dot_new = speed * jnp.sin(ang_new) - dt * y_dot * damping

            # updated positions
            x_new = jnp.clip(x + dt * x_dot_new, -x_max_arena, x_max_arena)
            y_new = jnp.clip(y + dt * y_dot_new, -y_max_arena, y_max_arena)
