...
BASE_ENERGY_RATE = 0.3  # base energy gain per timestep
...
class Sheep(Agent):
    ...
    def create_agent(type, params, id, active_state, key):
        ...
        def create_active_agent():
            ...
            state_content = {"x": x, "y": y, "ang": ang, "x_dot": x_dot, "y_dot": y_dot, "ang_dot": ang_dot, "energy": energy, "energy_offer": energy_offer}
        ...
        def create_inactive_agent():
            state_content = {"x": jnp.array([-1.0]), "y": jnp.array([-1.0]), "ang": jnp.array([0.0]),
                             ...
                             "energy": jnp.array([-1.0]), "energy_offer": jnp.array([-1.0])} # placeholder values
            state = State(content=state_content)
            return state
    ...
    def reset_agent(agent, reset_params):
        ...
        state_content = {"x": x, "y": y, "ang": ang, "x_dot": x_dot, "y_dot": y_dot, "ang_dot": ang_dot,
                         "energy": energy, "energy_offer": energy_offer}
        state = State(content=state_content)
...
def calculate_sheep_energy_intake(sheep: Sheep):
    ...
    num_sheep_in_radius, is_near_matrix = jax.vmap(
        sheep_local_density,
        in_axes=(0, None)
    )(sheep, sheep)
...
def main():
    ...
    sheep_xs_list, sheep_ys_list, sheep_angles_list = [], [], []
    wolf_xs_list, wolf_ys_list, wolf_angles_list = [], [], []
