...
2. Sensory System:
   - Foragers sense velocity (x_dot, y_dot, ang_dot) instead of position (x, y, ang)
   - More biologically realistic proprioception
   - Encourages reactive behaviors based on movement

3. Resource Competition:
   - Fair energy sharing: Energy divided equally among foragers in same patch
   - Prevents "first come, first served" resource exploitation
   - Encourages spread across resource landscape

4. Crowd Perception:
   - Changed is_in (boolean) to is_in_sum (count)
   - Allows foragers to sense local population density
   - Enables more sophisticated social behaviors

5. Patch Energy Management:
   - Changed from numerical energy_out to boolean is_energy_out
   - Simplifies patch energy depletion mechanics

   hyper params
   1) neurons increased to 100 from 40
   2) num of worlds reduced to 4 from 8
"""
import sys
sys.path.append('/home/sidcha/abmax')
...
# patch params
PATCH_AGENT_TYPE = 2
NUM_PATCH = 500#150 # env lumpiness
ENERGY_MAX = 5.0#10.0 # env lumpiness
PATCH_RADIUS = 5.0 # env lumpiness
GROWING_RATE = 0.1 # env lumpiness
EAT_RATE = 0.3 # env lumpiness

# training params
NUM_WORLDS = 4 # W&B update
